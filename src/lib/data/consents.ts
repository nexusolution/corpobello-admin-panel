...
// Audit trail captured at sign time (Etapa 4 compliance, migration 0035).
export type ConsentSignAudit = {
  ip?: string | null
  documentHash?: string | null
  userAgent?: string | null
}

/** Mark a consent as signed (status firmado + signer + timestamp + audit trail).
 *  The PDF is generated by the caller afterwards. */
export async function signConsent(
  id: string,
  signerName: string,
  signaturePath?: string | null,
  audit?: ConsentSignAudit,
): Promise<{ error: string | null }> {
  if (!isSupabaseConfigured()) return { error: null }
  const { error } = await getSupabase()
    ...
    .update({
      ...
      signed_ip: audit?.ip ?? null,
      document_hash: audit?.documentHash ?? null,
